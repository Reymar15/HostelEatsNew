...
<section class="admin-hero compact-admin-hero">
    <div>
        <p class="crumb">Super Admin — All Branches</p>
        <h2>Customer Management</h2>
        <p>View all customers across every branch. Filter by branch to drill down.</p>
    </div>
</section>

{{-- Branch Filter --}}
<form method="GET" action="{{ route('admin.customers') }}" style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:1.5rem;align-items:flex-end;">
    <label style="display:grid;gap:6px;font-weight:900;font-size:.8rem;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);">
        Filter by Branch
        <select name="branch_id" style="min-height:42px;padding:0 12px;border:1px solid var(--line);border-radius:8px;background:var(--panel);font-weight:800;color:var(--ink);" onchange="this.form.submit()">
    ...
    <a class="secondary-button" href="{{ route('admin.customers') }}">Reset</a>
</form>

{{-- Stat Cards --}}
<section class="admin-stat-grid store-stat-grid" style="margin-bottom:1.5rem;">
    @foreach ([
        ['label' => 'Total Customers',    'value' => $totalCustomers,     'icon' => 'C', 'tone' => 'blue',  'desc' => 'have placed at least one order'],
...
<section class="panel admin-table-gap" style="margin-bottom:1.5rem;">
    <div class="panel-head">
        <div>
            ...
            <p>Latest 10 orders placed across all branches.</p>
        </div>
    </div>
    <div class="table-wrap">
        <table class="history-table admin-data-table">
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="7" style="text-align:center;padding:2rem;color:#9ca3af;">No orders found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<section class="panel admin-table-gap">
    <div class="panel-head">
        <div>
            <h2>All Customers</h2>
            <p>{{ $totalCustomers }} customer{{ $totalCustomers !== 1 ? 's' : '' }} have placed orders across all branches.</p>
        </div>
    </div>

    <div class="admin-filter-bar">
        <input type="search" placeholder="Search name, email..." data-admin-search="all-customers-table">
        <select data-admin-filter="all-customers-table" data-filter-key="type">
    ...
    <div class="table-wrap">
        <table class="history-table admin-data-table" id="all-customers-table">
            <thead>
                <tr>
                    <th>Customer</th>
                    <th>Email</th>
...
<div class="admin-modal" data-modal-id="customer-profile-modal" aria-hidden="true">
    <div class="admin-modal-card" style="max-width:640px;">
        <header>
            <div>
                <p class="crumb">Customer Profile</p>
...
document.querySelectorAll('[data-admin-modal="customer-profile-modal"]').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        if (history.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#9ca3af;padding:1rem;">No orders found.</td></tr>';
            return;
        }
